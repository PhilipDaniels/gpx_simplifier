use std::{fs::{read_dir, File}, io::BufWriter, path::{Path, PathBuf}};
use quick_xml::{events::{attributes::Attribute, BytesStart, Event}, name::QName, Writer};
...
fn simplify(input_file: &Path) {
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Decl(decl)) => {
                println!("Got Decl: {}", String::from_utf8_lossy(&decl));
            ...
            Ok(Event::Start(e)) => {
                match e.name().as_ref() {
                    ...
                    b"trkpt" => {
                        ...
                        let lat2 = Attribute { key: QName(b"lat"), value: trimmed_lat.into() };
                        let lon2 = Attribute { key: QName(b"lon"), value: trimmed_lon.into() };
                    ...
                    b"time" => {
                        ...
                        match reader.read_event_into(&mut buf) {
                            Ok(Event::Text(t)) => {
                                writer.create_element("time")
                                .write_text_content(t)
                                .unwrap();
                            }
